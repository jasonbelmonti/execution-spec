...
- Author or revise: read `docs/execution-spec-template.md` and `docs/execution-spec-authoring-guide.md`, then use `docs/execution-spec-validation-profile.yaml` only for the final baseline structural validation pass.
- Review: read `docs/execution-spec-review-process.md` and the relevant template sections in `docs/execution-spec-template.md`.
- Example calibration: read `docs/examples/e1-bounded-docs-change-execution.md` when a concrete bounded-change example would reduce ambiguity.
...
- For authored or revised execution specs, run baseline deterministic Markdown validation before the final response:
  - Use `docs/execution-spec-validation-profile.yaml`.
  ...
  - Run `"${MARKDOWN_ENGINE_BIN_DIR:-$HOME/.local/bin}/markdown-engine" validate --file <spec.md> --profile <skill-dir>/docs/execution-spec-validation-profile.yaml --format json`.
